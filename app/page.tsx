      ...
      <section className="container pt-14 md:pt-20 pb-20 md:pb-24">
        <div className="grid lg:grid-cols-[1.08fr_.92fr] gap-10 lg:gap-14 items-center min-h-[650px]">
          <div className="max-w-[700px]">
            ...
            <h1 className="display mt-5 max-w-[760px]">{t.title1}<br /><span className="text-[#4E6A5B]">{t.title2}</span></h1>
            <p className="text-lg md:text-xl leading-8 text-black/65 max-w-xl mt-8">{t.description}</p>
            <div className="flex flex-wrap gap-3 mt-7">
              <Link href="/contact" className="btn btn-dark">{t.start} →</Link>
              <Link href="/portfolio" className="btn btn-light">{t.portfolioCta} →</Link>
            </div>
            <div className="grid sm:grid-cols-3 gap-5 mt-12 max-w-2xl">
              {t.benefits.map(([title, text]) => <div key={title} className="border-t border-black/15 pt-4"><div className="text-sm font-bold">{title}</div><p className="text-sm text-black/55 leading-6 mt-1">{text}</p></div>)}
            </div>
          </div>
      ...
      <section className="container py-20">
        ...
        <div className="grid md:grid-cols-3 gap-5 mt-12">
          {t.projects.map(([tag,title,description,image]) => <div key={tag} className="group">
            <div className="aspect-[4/3] rounded-[24px] bg-[#D9CDBB] overflow-hidden relative"><img src={image} alt={title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-transparent" /><div className="absolute top-6 left-6 text-xs font-bold text-white">{tag}</div></div>
