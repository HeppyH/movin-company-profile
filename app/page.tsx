...
const content = {
  en: {
    ...
    description: "MOVIN helps businesses create packaging that protects products, communicates value, and is ready for real-world production.", start: "Start a Project", what: "What We Do", whatTitle: "From packaging ideas to production-ready solutions.",
  ...
  id: {
    eyebrow: "Solusi Kemasan", title1: "KEMASAN", title2: "YANG MENGGERAKKAN.",
    description: "MOVIN membantu bisnis menciptakan solusi kemasan yang melindungi produk, memperkuat nilai merek, dan siap memenuhi kebutuhan produksi nyata.", start: "Mulai Proyek", what: "Layanan Kami", whatTitle: "Dari ide kemasan hingga solusi siap produksi.",
  ...
  return (
    <main>
      <section className="container pt-20 md:pt-28 pb-24">
        ...
        <div className="mt-16 rounded-[32px] overflow-hidden min-h-[430px] relative bg-[#D9CDBB]"><img src="/images/hero-packaging.jpg" alt="Professional MOVIN packaging solutions" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#16201D]/75 via-black/10 to-transparent" /><div className="absolute left-8 bottom-8 text-white font-black text-5xl md:text-7xl tracking-[-.06em] drop-shadow-lg">MOVIN<br />PACKAGING.</div></div>
      </section>
      ...
      <section className="container py-20"><div className="max-w-3xl"><div className="eyebrow">{t.capabilities}</div><h2 className="section-title mt-4">{t.capabilitiesTitle}</h2></div><div className="grid md:grid-cols-3 gap-5 mt-12">{t.projects.map(([tag,title,description,image])=><div key={tag} className="group"><div className="aspect-[4/3] rounded-[24px] bg-[#D9CDBB] overflow-hidden relative"><img src={image} alt={title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-transparent" /><div className="absolute top-6 left-6 text-xs font-bold text-white">{tag}</div></div><h3 className="text-xl font-bold mt-5">{title}</h3><p className="text-black/55 mt-2 leading-6">{description}</p></div>)}</div><Link href="/portfolio" className="btn btn-light mt-10">{t.portfolio} →</Link></section>
